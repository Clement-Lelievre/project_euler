...
def solve(input_: str) -> int:
    ...
    max_product = 0
    nb_rows, nb_cols = arr.shape
    # Compute the largest product of contiguous subarrays of length 4
    # Horizontally
    for i in range(nb_rows):
        for j in range(nb_cols - 3):
            product = np.prod(arr[i, j : j + 4])
            max_product = max(max_product, product)
    # Vertically
    for i in range(nb_rows - 3):
        for j in range(nb_cols):
            product = np.prod(arr[i : i + 4, j])
            max_product = max(max_product, product)
    # Diagonally (top-left to bottom-right)
    for i in range(nb_rows - 3):
        for j in range(nb_cols - 3):
            product = np.prod([arr[i + k, j + k] for k in range(4)])
            max_product = max(max_product, product)
