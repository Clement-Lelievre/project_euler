...
def word_to_index(word1: str, word2: str) -> frozenset:
    ...
    Args:
        word1 (str): a sequence of string characters
        word2 (str): an anagram of word1
    ...
    for char in min(word1, word2):
        ...
        ret += seen[char]
    for char in max(word1, word2):
        ret += seen[char]
    return ret


def solve(words: list[str]) -> int:
    """pass a list of words and get the highest square from a square anagram word pair

    Args:
        words (list[str]): the dictionary
    ...
    anagrams = {
        ...
        # and we must be able to use only digits 0 to 9
    }
    max_len = max(map(len, anagrams))
    cand_encoded = defaultdict(list)
    ...
    for v in squares.values():
        for comb in combinations(v, 2):
            squares_encoded[word_to_index(*map(str, comb))].extend([*comb])
    solution_squares = {
